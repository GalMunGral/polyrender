...
Path.prototype.draw = function(ctx2) {
  ctx2.beginPath();
  for (var i = 0; i < this.commands.length; i += 1) {
    var cmd = this.commands[i];
    if (cmd.type === "M") {
      ctx2.moveTo(cmd.x, cmd.y);
    } else if (cmd.type === "L") {
      ctx2.lineTo(cmd.x, cmd.y);
    } else if (cmd.type === "C") {
      ctx2.bezierCurveTo(cmd.x1, cmd.y1, cmd.x2, cmd.y2, cmd.x, cmd.y);
    } else if (cmd.type === "Q") {
      ctx2.quadraticCurveTo(cmd.x1, cmd.y1, cmd.x, cmd.y);
    } else if (cmd.type === "Z") {
      ctx2.closePath();
    }
  }
...
Glyph.prototype.draw = function(ctx2, x, y, fontSize, options) {
  this.getPath(x, y, fontSize, options).draw(ctx2);
};
Glyph.prototype.drawPoints = function(ctx2, x, y, fontSize) {
  function drawCircles(l, x2, y2, scale2) {
    ctx2.beginPath();
    for (var j = 0; j < l.length; j += 1) {
      ctx2.moveTo(x2 + l[j].x * scale2, y2 + l[j].y * scale2);
      ctx2.arc(x2 + l[j].x * scale2, y2 + l[j].y * scale2, 2, 0, Math.PI * 2, false);
    }
    ctx2.closePath();
    ctx2.fill();
  }
...
Glyph.prototype.drawMetrics = function(ctx2, x, y, fontSize) {
  var scale;
  x = x !== void 0 ? x : 0;
  y = y !== void 0 ? y : 0;
  fontSize = fontSize !== void 0 ? fontSize : 24;
  ...
  draw.line(ctx2, x, -1e4, x, 1e4);
  draw.line(ctx2, -1e4, y, 1e4, y);
  ...
  draw.line(ctx2, x + xMin * scale, -1e4, x + xMin * scale, 1e4);
  draw.line(ctx2, x + xMax * scale, -1e4, x + xMax * scale, 1e4);
  draw.line(ctx2, -1e4, y + -yMin * scale, 1e4, y + -yMin * scale);
  draw.line(ctx2, -1e4, y + -yMax * scale, 1e4, y + -yMax * scale);
  ctx2.strokeStyle = "green";
  draw.line(ctx2, x + advanceWidth * scale, -1e4, x + advanceWidth * scale, 1e4);
};
...
Font.prototype.draw = function(ctx2, text, x, y, fontSize, options) {
  this.getPath(text, x, y, fontSize, options).draw(ctx2);
};
Font.prototype.drawPoints = function(ctx2, text, x, y, fontSize, options) {
  this.forEachGlyph(text, x, y, fontSize, options, function(glyph, gX, gY, gFontSize) {
    glyph.drawPoints(ctx2, gX, gY, gFontSize);
  });
};
Font.prototype.drawMetrics = function(ctx2, text, x, y, fontSize, options) {
  this.forEachGlyph(text, x, y, fontSize, options, function(glyph, gX, gY, gFontSize) {
    glyph.drawMetrics(ctx2, gX, gY, gFontSize);
  });
};
